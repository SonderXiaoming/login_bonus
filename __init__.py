...
def get_pic(pic_path,grey):
    if PRELOAD:
        img = image_cache[pic_path]
    else:
        img = Image.open(os.path.join(hoshino.config.RES_DIR,'img', 'priconne','stamp',pic_path))
...
async def storage(bot, ev: CQEvent):
    ...
    for index,id in enumerate(cards_list):
        ...
        f = get_pic(id,False) if int(id[:-4]) in cards_num else get_pic(id,True)
